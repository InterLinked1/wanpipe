#class A10x
#for A101u, A102u, A101c and A102c cards
...
sub gen_wanpipe_conf{
	...
	open(FH, '/etc/wanpipe/samples/trixbox/templates/wanpipe.tdm.a10u') or die "Can open template";
	my $wp_file='';
       	while (<FH>) {
       		$wp_file .= $_;
	}
...
sub gen_zapata_conf{
	...
	while (<FH>) {
        ...
        close (FH);
        $zp_file.="\n";
	$zp_file.="\; Sangoma A".$self->card->card_model." port ".$self->fe_line."\n" ;
